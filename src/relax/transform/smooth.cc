...
class Annotator : public ExprMutator {
 public:
  ...
  Expr VisitExpr_(const CallNode* call_node) final {
    ...
    if (auto matched_expr = ExtractMatchedExpr(linear_pat_, new_e, bindings_)) {
    ...
    } else if (MatchExpr(smooth_linear_pat_, new_e, bindings_)) {
      ICHECK(1 == 2);
      ...
      auto gen_scale_param = [this](DataType dtype) -> Var {
        TensorStructInfo sinfo(ShapeExpr(Array<PrimExpr>()), dtype);
        String param_name = "scale_" + std::to_string(sm_counter_++);
        Var scale(param_name, sinfo);
        new_params_.push_back(scale);
        return scale;
      };

      DataType dtype = GetDataTypeFromTensor(lhs);
      Expr sm_lhs = smooth(lhs, /*a_scale*/ gen_scale_param(dtype), kSQActivation, "identity");
      Expr sm_rhs = smooth(rhs, /*w_scale*/ gen_scale_param(dtype), kSQWeight, "identity");

      return matmul(sm_lhs, sm_rhs, attrs->out_dtype);
    }
    return new_e;
  }
