 ...
 private:
  ...
  Expr MakeDequantize(Call call, Expr scale1, Expr scale2, DataType out_dtype) {
    ...
    if (out_dtype == DataType::Float(32)) {
      ...
      PrimValue min_value = PrimValue(Downcast<FloatImm>(tvm::min_value(out_dtype)));
      PrimValue max_value = PrimValue(Downcast<FloatImm>(tvm::max_value(out_dtype)));
      ...
      return astype(clip(out, min_value, max_value), out_dtype);
